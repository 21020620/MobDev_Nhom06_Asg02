...
fun LoginScreen(
        ...
        viewModel: LoginViewModel = koinViewModel(parameters = { parametersOf(navigator) })
) {
    ...
    LaunchedEffect(Unit) { if (viewModel.hasUser()) navigator.navigate(BookHomeScreenDestination) }
    val launcher =
    ...
    Surface {
        Column(modifier = Modifier.fillMaxSize().padding(start = 20.dp, end = 20.dp)) {
            Box(
                    modifier = Modifier.fillMaxWidth().defaultMinSize().weight(0.5f),
            ...
            Box(modifier = Modifier.fillMaxWidth().weight(0.3f)) {
                Title(title = "Hi, Welcome Back! ", subtitle = "Hello again, we missed you <3")
            }
            ...
            Column(
                ...
                Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.BottomEnd) {
                    PromptRow(
                            ...
                            onClick = { navigator.navigate(ResetPasswordScreenDestination) }
                    )
                }
            }

            Column(
                    modifier = Modifier.fillMaxWidth().weight(0.7f).padding(top = 16.dp),
